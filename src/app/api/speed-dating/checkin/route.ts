...
export async function POST(request: NextRequest) {
  try {
    ...
    const { data: event, error: eventError } = await supabase
      ...
      .eq('event_type', 'speed_dating')
      .single();

    if (eventError || !event) {
      return NextResponse.json({ error: 'Event not found or check-in closed' }, { status: 404 });
    }
    ...
    const seatNum = (checkinCount || 0) + 1;
    const isLate = event.status === 'active' ? 1 : 0;
    ...
    // TODO: Handle late arrival pairing logic if event is active
    ...
    console.error('Speed dating checkin error:', error);
