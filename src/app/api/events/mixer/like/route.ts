...
export async function POST(req: NextRequest) {
  ...
  // Check mutual
  const { data: theyLikedMe } = await admin.from('mixer_stars').select('id').eq('event_id', event.id).eq('starrer_id', targetId).eq('starred_id', user.id).maybeSingle();
  let matchId = null;
  if (theyLikedMe) {
  ...
  return NextResponse.json({ success: true, mutual: !!theyLikedMe, matchId });
}
